...
const FpContCountry: React.FC<PropsStepResTwo> = (props) => {
    ...
    const arrCountries = Country.getAllCountries().map(country => country.name);
    ...
    const addingCheckedCitiz: CheckedCountries[] = []
    const addingCheckedWoPer: CheckedCountries[] = []

    for (let i = 0; i < arrCountries.length; i++) {
        if (props.isFullPageCitizenship) {
            addingCheckedCitiz.push({ country: arrCountries[i], value: false })
        }
        else {
            addingCheckedWoPer.push({ country: arrCountries[i], value: false })
        }
    }

    const [isCheckedWoPer, setIsCheckedWoPer] = useState<CheckedCountries[]>(addingCheckedWoPer);
    const [isCheckedCitiz, setIsCheckedCitiz] = useState<CheckedCountries[]>(addingCheckedCitiz);
    ...
    const handleSetInputCountrySearch = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (value === '') {
            ...
            const newSearchedArrCountr = arrCountries.filter(el => el.includes(value));
            setStateArrCitiz(newSearchedArrCountr);
            setStateArrWoPer(newSearchedArrCountr);
        }
    };
    ...
    useEffect(() => {
        console.log(props.arrSelectedCitizenship)
    }, [props.arrSelectedCitizenship])
    ...
    useEffect(() => {
        ...
        if (props.isFullPageCitizenship && props.arrSelectedCitizenship && props.arrSelectedCitizenship.length > 0) {
            ...
            for (let i = 0; i < props.arrSelectedCitizenship.length; i++) {
                ...
                const existingCountryIndex = isCheckedUpd.findIndex(item => item.country === savedCountry);
    ...
    const listCountriesCitiz = stateArrCitiz.map((el, index) => {
        ...
        const checkedCitiz = isCheckedCitiz.find(item => item.country === el) || { country: el, value: false };
    ...
    const listCountriesWoPer = stateArrWoPer.map((el, index) => {
        ...
        const checkedWoPer = isCheckedWoPer.find(item => item.country === el) || { country: el, value: false };
        ...
        return (
            <div key={el} className="el-country" onClick={handleCheckboxChange}>
                <input
                    type="checkbox"
                    className={!isLimitLength ? 'checkbox-country_limit' : 'checkbox-country'}
                    checked={checkedWoPer?.value}
                />
                <span
